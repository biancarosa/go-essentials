# Próximos passos

Depois de concluir o projeto, você pode evoluí-lo sem alterar os objetivos deste livro:

1. adicionar validação de email e telefone;
2. permitir atualização de um contato mantendo o mesmo ID;
3. ordenar o resultado de `List`;
4. salvar e carregar contatos de um arquivo JSON;
5. criar testes table-driven;
6. adicionar uma interface de linha de comando;
7. expor as operações por HTTP em um curso posterior.

Evite fazer tudo ao mesmo tempo. Escolha uma mudança pequena, defina o comportamento esperado e escreva o código necessário para ela.

{% hint style="success" %}
Se você consegue explicar cada pacote, tratar os erros e evoluir o projeto sem copiar o exemplo final, concluiu os objetivos do Go Essentials.
{% endhint %}
